...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						<tr align="left" bgcolor="#F2FDFF">
							<td height="30" colspan="17" background="images/table_header.gif">注册用户列表</td>
						</tr>
					</table>
				</td>
			</tr>
				...
				%>
                            <tr align="center" bgcolor="#FFFFFF">
                             ...
                             <td>${u.yonghuming}</td>    <td>${u.mima}</td>    <td>${u.cheng}</td>    <td>${u.xingming}</td>    <td align='center' bgcolor='#ebf0f7'>${u.xingbie}</td>    <td width='90' align='center' bgcolor='#ebf0f7'><a href='${u.touxiang}' target='_blank'><img src='${u.touxiang}' width=88 height=99 border=0 /></a></td>    <td>${u.shengri}</td>    <td>${u.xingzuo}</td>    <td>${u.shenfenzheng}</td>    <td>${u.shouji}</td>    <td>${u.gexingqianming}</td>    <td>${u.aihao}</td>    <td align='center'>${u.issh} <input type="button" value="审核" onclick="javascript:location.href='sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=zhuceyonghu'"/></td>
                              ...
                              <td align="center"><a class="btn btn-info btn-small" href="doUpdateZhuceyonghu.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="deleteZhuceyonghu.do?id=${u.id }"
										onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a class="btn btn-info btn-small" href="zhuceyonghuDetail.do?id=${u.id}">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
		 ...
		 <c:if test="${page.page>1}">
              <a href="zhuceyonghuList.do?page=1" >首页</a>
             <a href="zhuceyonghuList.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="zhuceyonghuList.do?page=${page.page+1 }">下一页</a>
			<a href="zhuceyonghuList.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
